...
let commitTestOutput;

try {
  debug('Running Smoke Test');
  smokeTestOutput = execWithLog(`cd ../__external-test-cache/${externalProjectName} && ember test`);
} catch (e) {
...
try {
  execWithLog(
    `${useYarn ? 'yarn link ember-data' : 'npm link ember-data'}`
  );
} catch (e) {
  debug(e);
  throw new Error(
...
try {
  ...
  commitTestOutput = execWithLog(`cd ../__external-test-cache/${externalProjectName} && ember test`, true);
} catch (e) {
