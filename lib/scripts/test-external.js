...
if (!gitUrl) {
  throw new Error('No git url provided to `node ./lib/scripts/test-external`. An https git url should be the first argument.')
} else if (gitUrl.indexOf('https') !== 0) {
  throw new Error(`The git url provided to \`node ./lib/scripts/test-external\` should use https. Received '${gitUrl}'`);
}

console.log(`Testing external project ${externalProjectName} located at ${gitUrl} against this ember-data commit.`);
...
if (fs.existsSync(projectTempDir)) {
  ...
  try {
    ...
    debug(e);
    throw new Error(`Install of ${gitUrl} for external project ${externalProjectName} testing failed.`);
  }
}
...
try {
  execSync(`${useYarn ? 'yarn link' : 'npm link'} && cd ../__external-test-cache/${externalProjectName} && ${useYarn ? 'yarn link ember-data && yarn' : 'npm link ember-data && npm install'}`);
} catch (e) {
  debug(e);
